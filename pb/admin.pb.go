...
var fileDescriptor_73a7fc70dcc2027c = []byte{
	...
	0x70, 0x21, 0x21, 0x36, 0x2e, 0x26, 0x7f, 0x6f, 0x01, 0x06, 0x27, 0xb1, 0x13, 0x8f, 0xe4, 0x18,
	0x2f, 0x3c, 0x92, 0x63, 0x7c, 0xf0, 0x48, 0x8e, 0x71, 0xc6, 0x63, 0x39, 0x86, 0x28, 0x16, 0x3d,
	0xeb, 0x82, 0xa4, 0x24, 0x36, 0xb0, 0xc3, 0x8c, 0x01, 0x01, 0x00, 0x00, 0xff, 0xff, 0xf6, 0xdd,
	0x4a, 0x9c, 0xa7, 0x00, 0x00, 0x00,
}
...
func (m *AuthReq) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Cookie) > 0 {
		...
		i = encodeVarintAdmin(dAtA, i, uint64(len(m.Cookie)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *AuthAck) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *AuthAck) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *AuthAck) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *AuthAck) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: AuthAck: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: AuthAck: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
